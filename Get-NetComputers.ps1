function Get-NetComputers {
    ...
    [System.Net.Networkinformation.PingOptions]::$PingOptions.PingOptions(.1, $False)
    $ComputerInfo
   ...
   function Get-ComputerInfo {
        ...
        for([Int]$i = 0; $i -LT $NumIPs; $i++){
          $IPData = New-Object -TypeName PSObject
          $IP = $IPQueue.Dequeue()
          $Test = $Ping.Send($IP, 1, .1, $PingOptions)
          if($Test.Status -EQ 'Success'){
                $HostN = [System.Net.DNS]::GetHostEntry("$IP")
                $IPData | Add-Member -Type NoteProperty -Name IPAddress -Value $IP -Force
                $IPData | Add-Member -Type NoteProperty -Name HostName -Value $HostN.HostName -Force
